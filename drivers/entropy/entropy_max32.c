...
static int api_get_entropy(const struct device *dev, uint8_t *buffer, uint16_t length)
{
	return MXC_TRNG_Random(buffer, length);
}

static int api_get_entropy_isr(const struct device *dev, uint8_t *buffer, uint16_t length,
			       uint32_t flags)
{
	return 0;
}
