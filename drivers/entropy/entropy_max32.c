...
static int entropy_max32_get_entropy(const struct device *dev, uint8_t *buffer, uint16_t length)
{
	return MXC_TRNG_Random(buffer, length);
}

static int entropy_max32_get_entropy_isr(const struct device *dev, uint8_t *buffer, uint16_t length,
					 uint32_t flags)
{
	return 0;
}
...
static const struct entropy_driver_api entropy_max32_api = {
	.get_entropy = entropy_max32_get_entropy, .get_entropy_isr = entropy_max32_get_entropy_isr};
