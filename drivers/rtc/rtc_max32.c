			      ...
			      const struct rtc_time *timeptr)
{
	...
	while (MXC_RTC_GetTime(&crr_sec, &crr_subsec) != E_NO_ERROR) {
		;
	}
...
#define RTC_MAX32_INIT(_num)                                                                       \
	IF_ENABLED(CONFIG_RTC_ALARM, (static void max32_rtc_irq_init_##_num(void) {                \
			   IF_ENABLED(                                                             \
				   CONFIG_RTC_ALARM,                                               \
				   (IRQ_CONNECT(DT_INST_IRQN(_num), DT_INST_IRQ(_num, priority),   \
						rtc_max32_isr, DEVICE_DT_INST_GET(_num), 0);       \
	...
	static const struct max32_rtc_config rtc_max32_config_##_num = {                           \
		...
		IF_ENABLED(CONFIG_RTC_ALARM, (.irq_func = max32_rtc_irq_init_##_num, ))};          \
