...
static inline time_t convert_to_second(const struct rtc_time *timeptr)
{
	...
	s_tm.tm_sec = timeptr->tm_sec;
	s_tm.tm_min = timeptr->tm_min;
	s_tm.tm_hour = timeptr->tm_hour;
	s_tm.tm_mday = timeptr->tm_mday;
	s_tm.tm_mon = timeptr->tm_mon;
	s_tm.tm_year = timeptr->tm_year;
	s_tm.tm_wday = timeptr->tm_wday;
	s_tm.tm_yday = timeptr->tm_yday;
	s_tm.tm_isdst = timeptr->tm_isdst;

	sec = timeutil_timegm(&s_tm); /* Convert rtc_time to seconds*/
...
static inline void convert_to_rtc_time(uint32_t sec, uint32_t subsec, struct rtc_time *timeptr)
{
	...
	timeptr->tm_sec = s_tm.tm_sec;
	timeptr->tm_min = s_tm.tm_min;
	timeptr->tm_hour = s_tm.tm_hour;
	timeptr->tm_mday = s_tm.tm_mday;
	timeptr->tm_mon = s_tm.tm_mon;
	timeptr->tm_year = s_tm.tm_year;
	timeptr->tm_wday = s_tm.tm_wday;
	timeptr->tm_yday = s_tm.tm_yday;
	timeptr->tm_isdst = -1;
	timeptr->tm_nsec = 0;
	/* add subsec too */
	if (subsec >= (SUBSEC_PER_SEC / 2)) {
		timeptr->tm_sec += 1;
	}
}
				   ...
				   void *user_data)
{
	...
	while (MXC_RTC_SetSubsecondAlarm(sub_sec_alarm_max - SUBSEC_PER_SEC) == E_BUSY) {
		;
	}
