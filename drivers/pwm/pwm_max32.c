...
static int pwm_max32_set_cycles(const struct device *dev, uint32_t channel, uint32_t period_cycles,
...
static int pwm_max32_get_cycles_per_sec(const struct device *dev, uint32_t channel,
					uint64_t *cycles)
{
...
#define PWM_MAX32_DEFINE(i)                                                                        \
	...
	DEVICE_DT_INST_DEFINE(i, &pwm_max32_init, NULL, &max32_pwm_data_##i,                       \
			      &max32_pwm_config_##i, POST_KERNEL, CONFIG_PWM_INIT_PRIORITY,        \
