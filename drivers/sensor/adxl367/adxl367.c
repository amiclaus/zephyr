 ...
 * @return 0 in case of success, negative error code otherwise.
 */
static int adxl367_interrupt_config(const struct device *dev,
				    uint16_t int1,
				    uint16_t int2)
{
	int ret;
	...
	ret = data->hw_tf->write_reg(dev, ADXL367_INTMAP1_LOWER, int1 & 0xFF);
	if (ret)
		return ret;

	ret = data->hw_tf->write_reg(dev, ADXL367_INTMAP2_LOWER, int2 & 0xFF);
	if (ret)
		return ret;

	ret = data->hw_tf->write_reg(dev, ADXL367_INTMAP1_UPPER, int1 >> 8);
	if (ret)
		return ret;

	return data->hw_tf->write_reg(dev, ADXL367_INTMAP2_UPPER, int2 >> 8);
}
...
static int adxl367_probe(const struct device *dev)
{
	...
	ret = adxl367_interrupt_config(dev, cfg->int1_config, cfg->int2_config);
	if (ret) {
		return ret;
	}
